...
def create_review():
    """
    Create a review, track interaction, and index in search.
    """
    try:
        data = request.get_json() or {}
        ...
        # Track user interaction (best-effort)
        try:
            rec_engine = get_recommendation_engine(db)
        ...
        # Index the review in Elasticsearch (best-effort)
        try:
            if search_service.is_available:
...
# ----------------------------------------------------------------------
# TEMPORARY ADMIN SEED ENDPOINT (can be removed after seeding)
# ----------------------------------------------------------------------
@app.route("/api/admin/seed", methods=["GET", "POST"])
def admin_seed():
    """
    Temporary endpoint to seed categories, products, users, and reviews.
    Requires ?secret=SEED_SECRET or header X-Seed-Secret.
    ...
    try:
        ...
        created = {"categories": 0, "products": 0, "users": 0, "reviews": 0}
        ...
        for name, slug, icon in categories:
            _, new_cat = get_or_create(
                Category,
                name=name,
                slug=slug,
                defaults={"description": f"{name} category", "icon_url": icon},
            )
            if new_cat:
        ...
        return jsonify({"message": "Seed completed", "created": created}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
